...
>
	...
	>
		...
		<Field label="YEAR" placeholder="YYYY" error={errors.year?.[0] ?? null} bind:value={year} />
		{#if errors.root}
	...
	<section aria-label="Age calculator output">
		{#each derived as { label, value }}
			<div class="text-output italic">
				<span class="text-accent">
					{value ? value.toFixed().toString().padStart(2, '0') : '--'}
				</span>
				{label}
			</div>
		{/each}
